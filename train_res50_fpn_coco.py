...
import transforms
# import ..transforms
from modules.detectors import FasterRCNN, FastRCNNPredictor
from modules.backbones import resnet50_fpn_backbone
from modules.datasets import VOCDataSet, CocoDataset
...
def create_model(num_classes, device):
    ...
    weights_dict = torch.load("/home/gaoyun/Desktop/myfaster_rcnn/checkpoints/fasterrcnn_resnet50_fpn_coco.pth", map_location=device)
...
def main(parser_data):
    ...
    # check voc root
    if os.path.exists(os.path.join(COCO_root, "train2017")) is False:
    ...
    train_dataset = CocoDataset(ann_file=train_ann_file, data_root=train_path, transforms=data_transform["train"])
    ...
    val_dataset =  CocoDataset(ann_file=val_ann_file, data_root=val_path, transforms=data_transform["val"])
